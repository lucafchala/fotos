...
- **Navegação entre eventos**: botões "Anterior"/"Próximo" ao fim da página (antes do footer), na mesma ordem cronológica que a galeria mostra (`sortEvents()`, pinned primeiro depois data desc — não a ordem bruta de armazenamento no KV). Eventos ocultos (`visible:false`) são pulados na sequência. Se só existir vizinho de um lado, o grid vira uma coluna única (`.single-nav`) em vez de deixar metade vazia. Calculado em `handleEventPage()`, passado como `prevEvent`/`nextEvent` pra `eventHTML()`.
